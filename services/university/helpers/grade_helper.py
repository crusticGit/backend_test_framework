...
class GradeHelper(BaseHelper):
    ...
    def get_grade(self, student_id: int, teacher_id: int, group_id: int) -> requests.Response:
        response = self.api_utils.get(self.ROOT_ENDPOINT, params={
    ...
    def get_stats(self, student_id: int, teacher_id: int, group_id: int) -> requests.Response:
        response = self.api_utils.get(self.STATS_ENDPOINT, params={
            'student_id': student_id,
            'teacher_id': teacher_id,
            'group_id': group_id
        })
    ...
    def delete_grade(self, grade_id: int) -> requests.Response:
        response = self.api_utils.delete(self.GRADE_BY_ID_ENDPOINT.format(grade_id=str(grade_id)))
        return response

    def update_grade(self, grade_id: int, data: dict) -> requests.Response:
        response = self.api_utils.put(self.GRADE_BY_ID_ENDPOINT.format(grade_id=str(grade_id)), data=data)
        return response
